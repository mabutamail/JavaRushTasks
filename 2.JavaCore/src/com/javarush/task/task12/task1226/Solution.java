...
5. Добавь интерфейсы классам Animal.Cat(кот), Dog(собака), Tiger(тигр), Duck(Утка).
...
4. Объект класса Animal.Cat должен уметь бегать(поддерживать интерфейс CanRun) и лазить по деревьям(поддерживать интерфейс CanClimb).
